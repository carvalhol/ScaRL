program main_Stat
    ...
    character(len=1024) :: resPath, dset_name
    integer :: nFiles
    integer :: deleteSampleInTheEnd
    integer :: calculateCorrL
    !logical :: local_sample=.true.
    logical :: local_sample=.false.
    ...
    call MPI_BCAST (nFiles, 1, MPI_INTEGER, 0, comm, code)
    ...
    do i = 1, nFiles
        ...
        if(STA%rang == 0) then
            write(*,*) "File ", i
            read(*,*) resPath
            write(*,*) "path = ", trim(adjustL(resPath))
            read(*,*) dset_name
            write(*,*) "data_set_name = ", trim(adjustL(dset_name))
        end if
        ...
        if(STA%valid_proc) then
            ...
            call set_Local_Range(local_sample)
            call set_Sk_Dir()
            if(STA%rang == 0) write(*,*) "-> Reading HDF5 Tables"
            call read_RF_h5_File_Table(resPath, dset_name)
            ...
            if(STA%rang == 0) then
                call makeStatFile(resPath, STA%nDim, STA, deleteSampleInTheEnd)
            end if
        end if
    ...
    contains
        ...
        subroutine read_RF_h5_File_Attributes()
            ...
            if(STA%rang == 0) write(*,*) " Searching for file: ",trim(adjustL(resPath))
            ...
            !INT VEC
            attr_name = "Np"
            call read_h5attr_int_vec(file_id, attr_name, STA%Np)
            attr_name = "Np_ovlp"
            call read_h5attr_int_vec(file_id, attr_name, STA%Np_ovlp)
            attr_name = "L"
            call read_h5attr_int_vec(file_id, attr_name, STA%L)
        ...
        !---------------------------------------------------------------------------------
        !---------------------------------------------------------------------------------
        subroutine set_Local_Range(local_sample)
            implicit none

            !INPUT
            logical, intent(in) :: local_sample 
            !LOCAL
            integer :: topComm
            ...
            if(local_sample) then
                nPoints = STA%Np
            else
                nPoints = STA%L
            end if

            STA%localRange(:,1) = STA%coords * nPoints/STA%procPerDim + 1
            STA%localRange(:,2) = (STA%coords + 1) * nPoints/STA%procPerDim + 1

            where(STA%coords == STA%procPerDim-1)
                STA%localRange(:,2) = nPoints
            elsewhere
                STA%localRange(:,2) = STA%localRange(:,2) - 1
            end where

            STA%L_Loc = STA%localRange(:,2) - STA%localRange(:,1) + 1
    ...
    subroutine set_procPerDim_2 (STA, procPerDim)
        ...
        subroutine set_Sk_Dir()
            ...
            allocate(STA%Sk_Dir(sum(STA%L_Loc)))
            ...
            do i = 1, STA%nDim
                ...
                if(i == 1) then
                    ...
                    STA%Sk_Ind(i,1) = sum(STA%L_Loc(1:i-1)) + 1
                end if

                STA%Sk_Ind(i,2) = sum(STA%L_Loc(1:i))
        ...
        !---------------------------------------------------------------------------------
        !---------------------------------------------------------------------------------
        subroutine read_RF_h5_File_Table(file_path, dset)
            ...
            integer(HSIZE_T), dimension(3) :: offset, locDims
            ...
            if(STA%rang == 0) write(*,*) " Searching for file: ",trim(adjustL(resPath))
            call h5open_f(hdferr) ! Initialize FORTRAN interface.
            call h5fopen_f(trim(file_path), H5F_ACC_RDONLY_F, file_id, hdferr) !Open File
        ...
        !---------------------------------------------------------------------------------
        !---------------------------------------------------------------------------------
        subroutine makeStatFile(resPath, nDim_in, STA, deleteSampleInTheEnd)
            implicit none
            !INPUT
            character(len=*), intent(in) :: resPath
            ...
            integer :: error
            integer :: i
            character(len=buf_RF) :: newFile_Path, newFile_Folder, file_name
            character(len=buf_RF) :: old_file_base_name
            ...
            double precision, dimension(3) :: xMinGlob, xMaxGlob, xStep, corrL, overlap
            double precision, dimension(3) :: procExtent, kMax_out
            integer         , dimension(3) :: kNStep_out, nFields
            integer         , dimension(3) :: L, Np, Np_ovlp
            ...
            counter = 0
            sz_resPath = len(resPath)
            do i = sz_resPath, 1, -1
                if(resPath(i:i) == "/") then
                    counter = counter + 1 
                    if(counter == 1) then
                        file_name = resPath(i+1:sz_resPath)
                    ...
                    !    exit
                    !end if
                else if(resPath(i:i) == ".") then
                    old_file_base_name = resPath(1:i-1)
                end if
            end do
            newFile_Path =string_join_many(newFile_Folder,"STAT_",file_name)

           if(STA%rang == 0) write(*,*) "file_name          : ", trim(file_name)
           if(STA%rang == 0) write(*,*) "old_file_base_name : ", trim(old_file_base_name)
           ...
           if(STA%rang == 0) write(*,*) "newFile_Path       : ", trim(newFile_Path)
            ...
            if(deleteSampleInTheEnd == 1) then
                if(STA%rang == 0) then
                    ...
                    command = "rm "//trim(adjustL(old_file_base_name//"*"))
                    call system(command)
