module type_STAT
    ...
    type :: STAT
        ...
        integer , dimension(:), allocatable :: L, L_Loc, Np, Np_ovlp!, kNStep
    ...
    contains
        ...
        subroutine init_STAT(STAT_a, nDim, Nmc, method, corrMod, margiFirst, independent)
            ...
            allocate(STAT_a%L(nDim))
            allocate(STAT_a%Np(nDim))
            allocate(STAT_a%Np_ovlp(nDim))
            allocate(STAT_a%L_Loc(nDim))
        ...
        subroutine show_STAT(STAT_a, name, unit)
            ...
            write(unit,*) "  L              = ", STAT_a%L
            write(unit,*) "  Np             = ", STAT_a%Np
            write(unit,*) "  Np_ovlp        = ", STAT_a%Np_ovlp
        ...
        subroutine finalize_STAT(STAT_a)
            ...
            if(allocated(STAT_a%L))      deallocate(STAT_a%L)
            if(allocated(STAT_a%Np))      deallocate(STAT_a%Np)
            if(allocated(STAT_a%Np_ovlp))      deallocate(STAT_a%Np_ovlp)
            ...
            if(allocated(STAT_a%L_Loc))  deallocate(STAT_a%L_Loc)
