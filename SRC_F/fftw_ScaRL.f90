...
subroutine gen_std_gauss_Shino_FFT(data_real_3D, Np, &
    ...
    !Random phase
    allocate(r_phase(sz_Sk(1),sz_Sk(2),sz_Sk(3),4))
    ...
    c_in(1:sz_Sk(1),1:sz_Sk(2),1:sz_Sk(3)) = Sk_mtx*phiK(:,:,:,1)
    ...
    c_in(1:sz_Sk(1), &
         ...
         1:sz_Sk(3)) = Sk_mtx(:,sz_Sk(2):1:-1,:)*phiK(:,:,:,2)
    call dfftw_plan_dft_3d(plan, L,M,N, c_in,c_out,FFTW_BACKWARD, FFTW_ESTIMATE)
    call dfftw_execute_dft(plan, c_in, c_out)
    call dfftw_destroy_plan(plan)
    data_real_3D = data_real_3D + real(c_out(1:Np(1),1:Np(2),1:Np(3)))
    ...
    c_in(1:Np(1), &
         1:Np(2), &
         sz_c(3)-sz_Sk(3)+1:sz_c(3)) = Sk_mtx(:,:,sz_Sk(3):1:-1)*phiK(:,:,:,3)
    call dfftw_plan_dft_3d(plan, L,M,N, c_in,c_out,FFTW_BACKWARD, FFTW_ESTIMATE)
    call dfftw_execute_dft(plan, c_in, c_out)
    call dfftw_destroy_plan(plan)
    data_real_3D = data_real_3D + real(c_out(1:Np(1),1:Np(2),1:Np(3)))
    ...
    c_in(1:Np(1), &
         sz_c(2)-sz_Sk(2)+1:sz_c(2), &
         sz_c(3)-sz_Sk(3)+1:sz_c(3)) = Sk_mtx(:,sz_Sk(2):1:-1,sz_Sk(3):1:-1)*phiK(:,:,:,4)
    call dfftw_plan_dft_3d(plan, L,M,N, c_in,c_out,FFTW_BACKWARD, FFTW_ESTIMATE)
    call dfftw_execute_dft(plan, c_in, c_out)
    call dfftw_destroy_plan(plan)
    data_real_3D = data_real_3D + real(c_out(1:Np(1),1:Np(2),1:Np(3)))
